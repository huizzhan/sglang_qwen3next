...
import os
# os.environ["TRITON_CACHE_DIR"] = "/sgl-workspace/sglang/gluon_gemm_cache"
# os.environ["MLIR_ENABLE_DUMP"] = "1"
# os.environ["AMDGCN_ENABLE_DUMP"] = "1"

# 显示 Autotune 详细信息的环境变量选项
...
def matmul(a, b, show_config=False):
    M, K = a.shape
    _, N = b.shape
    ...
    # 显示 autotune 选择的配置
    if show_config and hasattr(matmul_kernel1, 'best_config'):
        print(f"\n{'='*60}")
        print(f"Autotune 选择的配置 (M={M}, N={N}, K={K}):")
        print(f"{'='*60}")
        best = matmul_kernel1.best_config
        if best:
            print(f"  BLOCK_SIZE_M: {best.kwargs.get('BLOCK_SIZE_M', 'N/A')}")
            print(f"  BLOCK_SIZE_N: {best.kwargs.get('BLOCK_SIZE_N', 'N/A')}")
            print(f"  BLOCK_SIZE_K: {best.kwargs.get('BLOCK_SIZE_K', 'N/A')}")
...
if __name__ == "__main__":
    ...
    # 第一次调用会触发 autotune
    print("运行 autotune...")
    triton_out = matmul(a, b, show_config=True)
