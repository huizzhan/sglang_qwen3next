...
@tl.core.builtin
def load_conv_weights(w_base, feats, stride_w_dim, stride_w_width, conv_width: int, mask=None, other=0.0, _semantic=None) -> tl.tuple:
    weights = [tl.load(w_base + i * stride_w_width, mask=mask, other=other)for i in range(conv_width)]
    return tl.tuple(weights)

@tl.core.builtin
def load_conv_states(conv_state_ptr, feats, stride_conv_state_dim, stride_conv_state_tok, conv_width: int, mask=None, other=0.0, _semantic=None) -> tl.tuple:
    states = [tl.load(conv_state_ptr + i * stride_conv_state_tok, mask=mask, other=other) for i in range(conv_width)]
...
def causal_conv1d_update_split_qkv(
    ...
    if use_gluon:
        kernel_fn = gluon_causal_conv1d_update_split_qkv_kernel_v2 if use_gluon_v2 else gluon_causal_conv1d_update_split_qkv_kernel
    else:
